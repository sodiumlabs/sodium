    ...
    <Pressable
      ...
      <MHStack style={[styles.container, globalStyle.whiteBorderWidth, { backgroundColor: isItemHovered ? eColor.GrayHover : '#ffffff' }]} stretchW>
        ...
        <MVStack style={{ flex: 1, marginLeft: 6 }}>
          ...
          <MLineLR
            style={{ marginTop: 2 }}
            left={<MText style={{ color: eColor.GrayText }} fontSize={10}>{formatWei2Price(tokenInfo.balance.toString(), tokenInfo.token.decimals)} {tokenInfo.token.symbol == "MELD" ? "USDC" : tokenInfo.token.symbol} ${tokenInfo.usdBalance}</MText>}
            right={<MText></MText>} />
        </MVStack>
      </MHStack>
    </Pressable>
